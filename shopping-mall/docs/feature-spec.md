...
본 문서는 Shoppping Mall Demo MVP 요구사항을 구현하기 위한 상세 기능 동작, 입력/출력, 예외 처리, 화면 구성을 정의한다.
...
- `PUT /api/users/me`: 사용자 프로필 업데이트

**검증 및 예외 처리 규칙:**

- 이름은 필수이며, 공백만 입력된 값은 허용하지 않는다.
- 이메일은 변경할 수 없다.
- 인증 토큰이 없거나 유효하지 않은 경우 401 오류 반환
...
- **BUY NOW 버튼**: 상품 디테일 페이지에서 즉시 구매 기능
  - 클릭 시 선택한 수량과 함께 주문 페이지로 이동
  - 주문 페이지에서 배송지 정보 입력 후 결제 진행
...
- 배송지 정보
  - 저장된 배송지 선택 또는 새 배송지 입력
  - 수령인 이름
  - 연락처
  - 우편번호
  - 주소 (다음 우편번호 API 사용 가능)
  - 상세주소
  - 배송 메모 (선택)
- 결제 수단
...
1. 로그인 상태 확인
2. 저장된 배송지 목록 조회 (있는 경우)
3. 기본 배송지 자동 선택 (있는 경우)
4. 재고 확인
5. 주문 금액 계산
6. 임시 주문 정보 저장
...
- **배송지 선택**: 저장된 배송지 목록에서 선택 가능
- **배송지 관리**: 주문 페이지에서 배송지 관리 페이지로 이동 가능
- **다음 우편번호 API**: 주소 검색 기능 제공
- **직접 입력**: 저장된 배송지 없이 새로 입력 가능

---

### 2.4.1-1 배송지 관리

**기능 설명:** 사용자가 배송지를 저장하고 관리
...
1. 로그인 상태 확인
2. 입력값 유효성 검증
3. 기본 배송지로 설정하는 경우, 다른 배송지들의 isDefault를 false로 변경
4. 배송지 저장/수정/삭제

**출력:**

- 성공: "배송지가 추가/수정/삭제되었습니다"
- 실패: 오류 메시지
...
- 수령인, 연락처, 우편번호, 기본 주소는 필수 입력
- 기본 배송지로 설정 시 다른 배송지의 기본 설정 해제
- 다음 우편번호 API를 통한 주소 검색 지원
- 직접 입력도 가능
...
- 관리자 권한이 없는 경우 접근 거부
- 존재하지 않는 회원 ID인 경우 "회원을 찾을 수 없습니다" 메시지 반환
- 탈퇴한 회원은 목록에서 제외 (isDeleted: false인 회원만 조회)
...
1. **메인 페이지**: 헤더(네비게이션바), 광고 배너, NEW ARRIVAL, BEST SELLERS, 푸터
2. **상품 목록 페이지**: 카테고리별/검색 결과 상품 목록
3. **상품 상세 페이지**: 상품 정보, 리뷰, 구매 버튼
4. **장바구니 페이지**: 장바구니 상품 목록, 주문하기
5. **주문서 페이지**: 배송지 선택/입력, 결제 수단 선택
6. **주문 완료 페이지**: 주문번호, 주문 정보
7. **마이페이지**: 주문 내역, 회원 정보, 리뷰 관리, 배송지 관리
8. **로그인/회원가입 페이지**
...
- 회원정보 수정 API 연동 완료 반영
- 배송지 관리 기능 상세 명세 추가
- 주문서 작성 시 배송지 선택 기능 추가
- 상품 상세 페이지 BUY NOW 기능 추가
